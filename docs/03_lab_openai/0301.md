...
1. After creating your Azure OpenAI instance, you need to deploy the language models you want. Use the following command to deploy the `gpt-4o` model:
   ...
   {: .note }
   > If the model create command fails with message like, "(InsufficientQuota) This operation require xx new capacity in quota Tokens Per Minute (thousands), ... which is bigger than the current available capacity yy", try again with a value within the available capacity using parameter --sku-capacity <new-value>.
